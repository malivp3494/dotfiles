...
parser.add_argument('-r', dest="print_only", action="store_true",
                    help="Do not download, just output the command only.")
...
def Main():
    link_flag, playlist_flag, video_quality, output_dir, audio_only, print_only = (
        args.link, args.playlist, args.quality, args.output_dir, args.audio_only, args.print_only)
    # pprint(args)
    ...
    cmd = f'youtube-dl -f {video_quality} -o {output_dir}/{video_title} {link_url} '
    if playlist_flag:
